...
pub fn encode_login_play(
    params: LoginPlayParams,
) -> io::Result<Packet> {
    let mut data = Vec::new();
    data.extend_from_slice(&params.entity_id.to_be_bytes()); // Entity ID
    data.push(if params.hardcore { 1 } else { 0 }); // Is hardcore
    VarInt(1).write(&mut data)?; // Dimension count
    write_string(&mut data, "minecraft:overworld")?; // Dimension name
    VarInt(params.max_players).write(&mut data)?; // Max players
    VarInt(params.view_distance).write(&mut data)?; // View distance
    VarInt(params.simulation_distance).write(&mut data)?; // Simulation distance
    ...
    data.push(params.game_mode as u8); // Game mode
    data.push(0xFF); // Previous game mode: -1
    data.push(0); // Is debug: false
    data.push(if params.is_flat { 1 } else { 0 }); // Is flat
    data.push(0); // Has death location
    VarInt(0).write(&mut data)?; // Portal cooldown
    VarInt(params.sea_level).write(&mut data)?; // Sea Level
...
mod tests {
    ...
    fn test_login_play_protocol_775() {
        let packet = encode_login_play(LoginPlayParams {
            entity_id: 1,
            view_distance: 8,
