...
export default function PacketReference() {
  ...
  const [selectedPhase, setSelectedPhase] = useState<
    "All" | "Status" | "Login" | "Configuration" | "Play"
  >("All");
  const [selectedDirection, setSelectedDirection] = useState<"All" | "Clientbound" | "Serverbound">(
    "All",
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm">{t("packet_ref.desc")}</p>
        </div>
      </div>
        ...
        {filteredPackets.length > 0 ? (
          filteredPackets.map((packet) => {
            ...
            const translatedDesc = t(
              `packets.${packet.phase}.${packet.name}.description`,
              packet.description,
            );
              ...
              >
                <div>
                  ...
                  <div className="flex items-baseline justify-between mb-2">
                    ...
                    <span className="text-sm font-mono text-cyan-400 font-bold">{packet.id}</span>
                  </div>

                  <p className="text-slate-400 text-sm leading-relaxed mb-4">{translatedDesc}</p>
                </div>
                ...
                <div className="mt-2 pt-3 border-t border-slate-800/60">
                  {packet.fields.length > 0 ? (
                    <div>
                      ...
                      >
                        {isExpanded ? t("packet_ref.actions.hide") : t("packet_ref.actions.show")} (
                        {packet.fields.length})
                      </button>
                          ...
                          <table className="w-full text-left text-xs border-collapse">
                            ...
                            <tbody className="divide-y divide-slate-800/40 text-slate-400">
                              {packet.fields.map((field) => {
                                const translatedFieldDesc = t(
                                  `packets.${packet.phase}.${packet.name}.fields.${field.name}`,
                                  field.description,
                                );
                                return (
