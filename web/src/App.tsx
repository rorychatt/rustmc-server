    ...
    <div className="min-h-screen bg-[#080d16] text-slate-100 flex flex-col relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
              {t('header.title')} <span className="text-xs bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 px-2 py-0.5 rounded font-mono font-bold">{t('header.version')}</span>
            </h1>
      ...
      <div className="flex-grow max-w-7xl w-full mx-auto px-6 py-8 grid grid-cols-1 md:grid-cols-12 gap-8 items-start relative z-10">
        ...
        <aside className="hidden md:block md:col-span-3 sticky top-24 space-y-2">
          <div className="text-[10px] uppercase font-bold text-slate-500 tracking-widest pl-3 mb-3">{t('nav.navigation')}</div>
          {tabs.map(tab => {
            const Icon = tab.icon;
            const isActive = activeTab === tab.id;
            return (
        ...
        <main className="md:col-span-9 space-y-8">
            ...
            <div className="space-y-8 animate-fadeIn">
              ...
              <div className="relative rounded-2xl overflow-hidden border border-slate-800/80 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 p-8 md:p-10 shadow-2xl">
                ...
                <div className="max-w-2xl space-y-4">
                  ...
                  <h2 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight leading-tight">
                    {t('overview.hero.title')} <span className="text-gradient-cyan">{t('overview.hero.title_highlight')}</span>
                  </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="p-6 rounded-xl glass-panel glass-panel-hover space-y-2">
                  ...
                  <h3 className="text-base font-bold text-white">{t('overview.features.tokio.title')}</h3>
                ...
                <div className="p-6 rounded-xl glass-panel glass-panel-hover space-y-2">
                  ...
                  <h3 className="text-base font-bold text-white">{t('overview.features.config.title')}</h3>
                  <p className="text-slate-400 text-xs leading-relaxed">
                ...
                <div className="p-6 rounded-xl glass-panel glass-panel-hover space-y-2">
                  ...
                  <h3 className="text-base font-bold text-white">{t('overview.features.transfer.title')}</h3>
                  <p className="text-slate-400 text-xs leading-relaxed">
              ...
              <div className="p-6 rounded-xl glass-panel space-y-4">
                ...
                <div className="bg-slate-950/60 rounded-lg border border-slate-800 p-4 font-mono text-xs text-emerald-400/90 space-y-2 overflow-x-auto">
                  <div><span className="text-slate-500">{t('overview.quickstart.clone')}</span></div>
                  <div>git clone https://github.com/rorychatt/rustmc-server.git</div>
                  <div>cd rustmc-server</div>
                  <div><span className="text-slate-500">{t('overview.quickstart.build')}</span></div>
                  <div>cargo build --release</div>
                  <div><span className="text-slate-500">{t('overview.quickstart.run')}</span></div>
          ...
          {activeTab === 'architecture' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="p-6 rounded-xl glass-panel space-y-4">
                <h3 className="text-lg font-bold text-white">{t('architecture.broadcasting.title')}</h3>
                <p className="text-slate-400 text-sm leading-relaxed">
                ...
                <div className="p-4 rounded-lg bg-slate-950/40 border border-slate-800 text-xs space-y-2">
                  <div className="font-mono text-cyan-400">{t('architecture.broadcasting.flow_title')}</div>
                    ...
                    <ul className="list-disc list-inside space-y-2 pl-2">
                      <li>
                        <Trans i18nKey="world.generators.flat">
                          <strong>Flat World (<code>flat</code>):</strong> Generates a uniform layered terrain consisting of 1 Bedrock, 2 Dirt, and 1 Grass block at the configured <code>sea_level</code> (default Y=63). Ideal for testing and lightweight lobby setups.
                        </Trans>
                      </li>
                      <li>
                        <Trans i18nKey="world.generators.normal">
                          <strong>Normal World (<code>normal</code>):</strong> Utilizes a noise generator based on a pseudo-random number seed (specified by <code>seed</code>) to produce natural terrain contours, hills, and valleys that emulate standard Minecraft environments.
                        </Trans>
                      </li>
                    </ul>
                  </div>
              ...
              <div className="p-6 rounded-xl glass-panel space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                  <div className="p-4 rounded-lg bg-slate-950/40 border border-slate-800/60 text-xs space-y-2">
                    <div className="font-bold text-white font-mono">{t('world.persistence.meta_title')}</div>
                    <p className="text-slate-400 leading-normal">
                      {t('world.persistence.meta_desc')}
                    </p>
                  </div>
                  <div className="p-4 rounded-lg bg-slate-950/40 border border-slate-800/60 text-xs space-y-2">
                    <div className="font-bold text-white font-mono">{t('world.persistence.chunk_title')}</div>
                    <p className="text-slate-400 leading-normal">
                      {t('world.persistence.chunk_desc')}
                    </p>
                  </div>
                  <div className="p-4 rounded-lg bg-slate-950/40 border border-slate-800/60 text-xs space-y-2">
                    <div className="font-bold text-white font-mono">{t('world.persistence.io_title')}</div>
                    <p className="text-slate-400 leading-normal">
                ...
                <div className="bg-slate-950/60 rounded-lg border border-slate-800 p-4 font-mono text-xs text-emerald-400/90 space-y-1 overflow-x-auto">
                  ...
                  <div>world/</div>
                  <div>├── level.json                 <span className="text-slate-500">{t('world.persistence.comment_meta')}</span></div>
                  <div>└── chunks/                   <span className="text-slate-500">{t('world.persistence.comment_chunks')}</span></div>
                  <div>    ├── c.0.0.json.zlib        <span className="text-slate-500">{t('world.persistence.comment_c00')}</span></div>
                  <div>    ├── c.0.1.json.zlib        <span className="text-slate-500">{t('world.persistence.comment_c01')}</span></div>
                  <div>    └── c.-1.0.json.zlib       <span className="text-slate-500">{t('world.persistence.comment_c10')}</span></div>
                </div>
              </div>
              ...
              <div className="p-6 rounded-xl glass-panel space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-5 rounded-lg bg-slate-950/40 border border-slate-800/60 space-y-3">
                    <h4 className="text-sm font-bold text-white">{t('world.tasks.autosave_title')}</h4>
                    <p className="text-slate-400 text-xs leading-relaxed">
                  ...
                  <div className="p-5 rounded-lg bg-slate-950/40 border border-slate-800/60 space-y-3">
                    <h4 className="text-sm font-bold text-white">{t('world.tasks.backup_title')}</h4>
                    <p className="text-slate-400 text-xs leading-relaxed">
              ...
              <div className="p-6 rounded-xl glass-panel space-y-3">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 py-6 px-4 rounded-lg bg-slate-950/40 border border-slate-800 text-xs font-mono text-center">
                  <div className="w-full md:w-1/5 p-3 rounded-lg border border-cyan-500/20 bg-cyan-950/10 text-cyan-300">
                    <span className="block font-bold">{t('protocol_tab.flow.handshake_title')}</span>
                    <span className="text-[10px] text-slate-500">{t('protocol_tab.flow.handshake_desc')}</span>
                  </div>
                  ...
                  <div className="w-full md:w-1/5 p-3 rounded-lg border border-emerald-500/20 bg-emerald-950/10 text-emerald-300">
                    ...
                    <span className="text-[10px] text-slate-500">{t('protocol_tab.flow.status_desc')}</span>
                  </div>
                  <div className="text-slate-500">OR ➔</div>
                  <div className="w-full md:w-1/5 p-3 rounded-lg border border-cyan-500/20 bg-cyan-950/10 text-cyan-300">
                    <span className="block font-bold">{t('protocol_tab.flow.login_title')}</span>
                    <span className="text-[10px] text-slate-500">{t('protocol_tab.flow.login_desc')}</span>
                  </div>
                  <div className="text-slate-500">➔</div>
                  <div className="w-full md:w-1/5 p-3 rounded-lg border border-cyan-500/20 bg-cyan-950/10 text-cyan-300">
                    <span className="block font-bold">{t('protocol_tab.flow.play_title')}</span>
                    <span className="text-[10px] text-slate-500">{t('protocol_tab.flow.play_desc')}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="p-5 rounded-xl glass-panel space-y-2">
                  ...
                  <h3 className="text-sm font-bold text-white">{t('transfer_tab.steps.negotiate_title')}</h3>
                  <p className="text-slate-400 text-xs leading-relaxed">
                ...
                <div className="p-5 rounded-xl glass-panel space-y-2">
                  ...
                  <h3 className="text-sm font-bold text-white">{t('transfer_tab.steps.store_title')}</h3>
                  <p className="text-slate-400 text-xs leading-relaxed">
                ...
                <div className="p-5 rounded-xl glass-panel space-y-2">
                  ...
                  <h3 className="text-sm font-bold text-white">{t('transfer_tab.steps.handoff_title')}</h3>
                  <p className="text-slate-400 text-xs leading-relaxed">
          ...
          {activeTab === 'security' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Rate Limiting */}
                <div className="p-5 rounded-xl glass-panel space-y-3">
                  <h3 className="text-base font-bold text-white">{t('security_tab.rate_limiting.title')}</h3>
                  <p className="text-slate-400 text-xs leading-relaxed text-slate-400">
